...
const TubeFill = props => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 338.057 52.271" className={props.className}>
      <defs>
        <linearGradient id={"linear-gradient--result-tube" + props.colors[0] + props.colors[1]} x1="0.5" x2="0.5" y2="1" gradientUnits="objectBoundingBox">
          <stop offset="0" stop-color={props.colors[0]} />
          <stop offset="1" stop-color={props.colors[1]} />
        </linearGradient>
      </defs>
      <path id="Path_20808" data-name="Path 20808" d="M-53.958-554.049h0A26.139,26.139,0,0,1-27.82-580.187H276.319c-2.566,4.817-4.412,9.64-3.489,15.024a29.764,29.764,0,0,0,6.181,13.423c.361.447.7.921,1.046,1.395,3.394,4.721,5.439,10.223,2.918,15.761-.589,1.3-3.528,6.595-5.61,6.595-.117,0-278.216.044-305.162.074A26.131,26.131,0,0,1-53.958-554.049Z" transform="translate(53.958 580.187)" fill={`url(#${"linear-gradient--result-tube" + props.colors[0] + props.colors[1]})`} />
    </svg>
  )
}
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tube}>
        {props.noAnimate && (
          <div>
            <TubeFillTotal className={styles.tubeFillSVGFull} />
          </div>
        )}
        <motion.div
          style={{ display: 'flex', flexDirection:'row', x: xSpring }}
        >
          {!props.noAnimate && (
            <TubeFill className={styles.tubeFillSVG} colors={props.colors} />
          )}
          {!props.noAnimate && (
            <Flake className={styles.flake} colors={props.flakeColors} />
          )}
        </motion.div>
      </div>
      <div className={styles.scoreLabel} style={{ color: props.textColor }}>
        {(props.noAnimate && props.fill === 1)
          ? 0
          // : maxCap(Math.floor(getText(xState)), props.fill)
          : props.fill
        }
      </div>
    </div>
