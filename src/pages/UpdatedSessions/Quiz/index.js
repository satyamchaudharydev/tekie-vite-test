...
const getQuizReportId = (props) => {
    ...
    if (quizReportTopicId && (props.match.path === '/sessions/:courseId/:topicId/quiz')) {
...
const mapStateToProps = (state, props) => {
    ...
    const quizReportTopicId = getQuizReportId(props)
    const navigationId = quizReportTopicId || props.match.params.topicId
    const userQuizs = getUserQuizs(
        state.data.getIn(['userQuiz', 'data']),
        `userQuiz/${navigationId}`
    ).get('quiz')
    ...
    return ({
        ...
        ...fetchQuizReport(navigationId, courseId).mapStateToProps(state),
        userQuizAnswers: state.data.getIn([
            'userQuizAnswers',
            'data'
        ]),
        userQuizReportStatus: getQuizReportStatus(state, navigationId),
