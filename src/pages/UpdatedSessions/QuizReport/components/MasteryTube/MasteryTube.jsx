...
const MasteryTube = ({ shouldAnimate = true, ...props }) => {
  ...
  const motionInitialValue = (masteryValue) => {
    if (!isMobile()) {
      return !shouldAnimate ? {
        x: (mastery >= masteryValue) ? hs(136.5) : 0,
      } : {}
    }
    return !shouldAnimate ? {
      x: (mastery >= masteryValue) ? 90 : 0,
    } : {}
  }

  const motionAnimateValue = (masteryValue) => {
    if (!isMobile()) {
      return {
        x: (mastery >= masteryValue) ? hs(136.5) : 0,
        transition: {
  ...
  return (
    <div className={cx(styles.masteryTubeContainer, customStyles.masteryTubeContainer)}>
      <div className={cx(styles.masteryTubeLeft, customStyles.masteryTubeLeft)}>
        <motion.div className={styles.fillLeft} initial={motionInitialValue(1)} animate={motionAnimateValue(1)}>
      ...
      <div className={cx(styles.masteryTubeCenter, customStyles.masteryTubeCenter)}>
        <motion.div initial={motionInitialValue(2)} className={styles.fillCenter} animate={motionAnimateValue(2)}>
          {bubbles.map((bubble, i) => (
            <Bubble
              bubble={{ ...bubble, delay: delays[random(0, delays.length)] }}
      ...
      <div className={cx(styles.masteryTubeRight, customStyles.masteryTubeRight)}>
        <motion.div initial={motionInitialValue(3)} className={styles.fillRight} animate={!isMobile() ? {
          x: (mastery === 3) ? hs(136.5) : 0,
          transition: {
            duration: 0.8,
            type: 'tween'
          }
