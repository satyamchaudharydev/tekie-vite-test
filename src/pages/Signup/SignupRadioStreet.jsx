...
import { countriesAllowed, eventSchoolList } from '../../config'
...
import getCountry, { getCity } from '../../utils/getCountry.js'
...
const renderCheckMark = () => (
  <svg className='signup-page-radiostreet-checkmark' viewBox='0 0 80 80' fill="none">
...
class Signup extends Component {
  ...
  signUp = async ({
    name, email, schoolName, grade, parentName, countryCode, phone
  }) => {
    const isB2C = this.props.history.location.pathname === '/signup/spy-squad-camp-community'
    const isCrackTheCode = this.props.history.location.pathname === '/signup/crack-the-code'
    ...
    let menteeInput = {
      ...
      country: getCountry(),
      city: getCity(),
      utmSource: isB2C ? 'communityevent' : 'events',
      utmCampaign: isB2C ? 'spysquadcamp_18dec' : 'crackthecode',
    ...
    if (window.location.search.includes('utm_source')) {
      menteeInput.utmSource = qs.parse(window.location.search).utm_source
    }
    if (window.location.search.includes('utm_campaign')) {
      menteeInput.utmCampaign = qs.parse(window.location.search).utm_campaign
    }
    if (window.location.search.includes('utm_medium')) {
      menteeInput.utmMedium = qs.parse(window.location.search).utm_medium
    }
    if (window.location.search.includes('utm_term')) {
      menteeInput.utmTerm = qs.parse(window.location.search).utm_term
    }
    if (window.location.search.includes('utm_term')) {
      menteeInput.utmContent = qs.parse(window.location.search).utm_content
    }
    if (!isB2C) {
      menteeInput.schoolName = schoolName.value ? schoolName.value : schoolName
    }
    await signUpMentee(menteeInput, null, null, () => {
      this.setState({ isLoading: false, shouldOTP: true })
    }).call()
      ...
      <>
        ...
        <div className={'signup-page-tekieFeatureContainer'} style={{ pointerEvents: slide === 1 ? 'auto' : 'none' }}>
          ...
          <div className={cx('signup-page-TFrow', 'signup-page-yourStoryContainer')}>
            ...
            <a href="https://medium.com/gsv-ventures/tekie-where-coding-meets-storytelling-d8a63d224b6d" target='_blank'>
    ...
    return (
      <Formik
        initialValues={{
          ...
          phone: `${get(this.props, 'location.state.phone.countryCode', '91')}${get(this.props, 'location.state.phone.number', '')}`,
          role: '',
          schoolName: isB2C ? 'Empty' : '',
          parentName: '',
          countryCode: get(this.props, 'location.state.phone.countryCode', '+91')
        }}
        validationSchema={validationMentee}
        onSubmit={({ name, email, countryCode, phone, schoolName, parentName }) => {
          this.signUp({
            name, email, countryCode, phone, schoolName, parentName
          })
        }}
      >
        {({ errors, touched, isValidating, handleSubmit, values, handleChange, setValues, validateForm }) => (
          <Form style={{ width: '100%', display: 'flex', flexDirection: 'column' }} errors={errors}>
            ...
            {!isB2C && (
              <div className={'signup-page-inputColumn'}>
                <Select
                  ...
                  value={Object.keys(this.state.school).length > 0 ? this.state.school : null}
            ...
            {this.state.school.value === 'other' && (
              <div className={'signup-page-inputColumn'}>
                <Field
                  ...
                  className={cx('signup-page-input', errors.schoolName && touched.schoolName && 'signup-page-inputError')}
                />
                <div className={'signup-page-error'}>
                  {errors.schoolName && touched.schoolName && errors.schoolName}
                </div>
            ...
            <div className='signup-page-checkbox-row'>
              <CheckBox
                value={this.state.hasLaptopOrDesktop}
                onChange={() => {
                  this.setState({
                    hasLaptopOrDesktop: true
                  })
                }}
                container='signup-page-checkbox-container'
              /><span>Yes</span>
              <CheckBox
                value={!this.state.hasLaptopOrDesktop}
                onChange={() => {
                  this.setState({ hasLaptopOrDesktop: false })
                }}
                container='signup-page-checkbox-container-last'
              /><span>No</span>
            </div>
  ...
  render() {
    ...
    return (
      <>
        <div className={cx('signup-page-modal-container', this.state.isPopup && 'show')}>
          <div
            className={'signup-page-modal-wrapper'}
          >
            {/* <div className='signup-page-close-btn'>
              <div>x</div>
            </div> */}
            {renderCheckMark()}
            <div className='signup-page-congrats'>Congratulations!</div>
            <div className='signup-page-spy-text'>You’re one step closer to becoming a spy! 🔍<br />We will share more details about the event shortly.</div>
        ...
        <div className={'signup-page-container'}>
          ...
          <div className={'signup-page-body'}>
            ...
            <div className={'signup-page-headingBookSession'}>Join {isCrackTheCode ? 'Crack The Code' : 'Spy Squad Camp'}</div>
            <div className={'signup-page-laptopText'}>Participate and stand a chance to win exciting prizes!</div>
          ...
          <motion.div
            initial={{
              opacity: this.state.shouldOTP ? 1 : 0
            }}
            animate={{
              opacity: this.state.shouldOTP ? 1 : 0
            }}
            style={{
              pointerEvents: this.state.shouldOTP ? 'auto' : 'none'
            }}
          >
            <Login
              shouldRedirect={false}
              visible={this.state.shouldOTP}
              otpDefault
              closeLoginModal={() => {
                this.setState({ shouldOTP: false })
              }}
              ctaText='Verify OTP'
              onChangeNumber={() => {
                this.setState({ shouldOTP: false })
              ...
              postLogin={() => {
                this.setState({ isPopup: true, shouldOTP: false })
              }}
              titleText='Enter Your OTP'
              hideRegisterNow
              phoneNumber={this.state.phone.replace(this.state.countryCode, '')}
